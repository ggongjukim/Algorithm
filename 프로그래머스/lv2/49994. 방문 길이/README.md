...
### 느낀점
a->b 길을 b->a 가더라도 같은 길을 가는 것이다    
따라서 `a.append(b)` `b.append(b)` 이렇게 양쪽으로 넣어줘야함    
인접리스트로 그래프 만드는 것이 핵심이었던거 같음
중복인지 아닌지 할때는 set을 사용하면 도움이 된다
그냥 하라는 대로 하면 되기때문에 구현? 이지 않을까 
